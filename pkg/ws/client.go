...
func (c *Client) readPump() {
	...
	for {
		...
		if c.toServer != nil {
			...
			log.Printf("toServer Channel is nil, message will be ignored")
		}
...
func write(c *Client, message []byte) error {
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			log.Printf("error writing c.toClient: %v", err)
...
func closeWebsocket(c *Client) {
	...
	if err != nil {
		log.Printf("error writing CloseMessage: %v", err)
	}
}
